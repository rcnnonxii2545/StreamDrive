  ...
  useEffect( () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        setData1(response.data.data.Bus);
    ...
    const suggestionData = data1.map(item => ({
      label: `${item.Name}  Busnumber :${item.Busnumber}`,
      value: `${item.Name}`,
      value1: `${item.Busnumber}`,
    }));
    ...
    <View style={styles.BG}>
        <View>
          ...
          <View style={{  padding: 20, borderRadius: 20 }}>
            <Dropdown
              ...
              onChange={(item) => {
                setName(item.value);
                setBusnumber(item.value1)
                setIsFocus(false);
              }}
            />
          </View>
          <Image
          source={require('./image/bus-driver.png')} 
          style={styles.image}
        />
        </View>
        ...
        {/* {Object.values(latestLocationDataByBusnumber).map((latestLocationData) => (
          ...
          <MyButton
            ...
            style={{ position: 'absolute', bottom: 0 }}
            title="Next"
            onPress={() => navigation.navigate('Startstop2', {Name,Busnumber})}
