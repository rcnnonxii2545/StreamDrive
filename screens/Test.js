...
const Test = () => {
  ...
  const [pointAnnotations,setPointAnnotations] = useState(null);
  ...
  function makeRoute(coordinates) {
    ...
    const filteredData1 = data.filter((item) => item.BusID.Busnumber === busnumber);
    ...
    //   console.log("last",bus.Latitude)
    // })
    // console.log(latestLocationDataByBusnumber)
    ...
    async function createRouterline() {
        if (userLocation && filteredData.length > 0) {
          ...
          for (let i = 0; i < filteredData.length - 1; i++) {
            ...
            // Object.values(latestLocationData).map(async (bus) => {

            // })
            Object.values(latestLocationDataByBusnumber).map( async (bus) => {
              
              console.log("last",bus.Latitude,bus.Longitude)
              if (userLocation) {
                const startCoords = `${bus.Longitude},${bus.Latitude}`;
            ...
            <MapboxGL.MapView   
                ...
                {Object.values(latestLocationDataByBusnumber).map((bus) => (
                    ...
                    >
                      ...
                      <Icon name="bus" size={27} color="white" />
                      </View>
                    </MapboxGL.PointAnnotation>
                  ))}
                ...
                >
                    <View style={styles.destinationIcon1}>
                    <Ionicons name="bus-outline" size={20} color="blue"/>
                    </View>
                </MapboxGL.PointAnnotation> */}
